...
The "overview tab will give a brief overview of network concepts, most of which were discussed here. The "network metadata" tab will start to address your network, going over network level properties such as the number of nodes and edges, as well as the afromentioned directed and weighted options. It will also give network level metrics such as the density and centralization measures, along with an explanation of how to interpret these scores. A general network vidualization is also provided, with the ability to mouse over nodes and see more information about them. Please check over the information here for errors.
