...
## load in network
rgraph = readRDS("./data/bis2a_net.rds")
...
custom_tooltips = lapply(seq_along(custom_tooltips), function(i){
  ...
  paste0(
    "<tr>",
    "<td>", names(custom_tooltips)[[i]], " </td>",
    "<td>", custom_tooltips[[i]], "</td>",
    "</tr>"
  )
...
`r if(!is.na(table(unique(c(vnet$edges$from, vnet$edges$to)) %in% vnet$nodes$id)["FALSE"])){paste("* There are nodes in the edges data with no corresponding node in the attributes data.")}`
...
overview_net$title = paste0("<table>",
                              ...
                              "</tr>",
                            if(all(!is.na(custom_tooltips))){
                              apply(data.frame(custom_tooltips, stringsAsFactors = FALSE), 1, FUN = function(trow){paste(trow, collapse = " ")})
                            },
                            "</table>")
...
This report was coded by [Jared Joseph](https://jnjoseph.com/) for the [UC Davis DataLab](https://datalab.ucdavis.edu/); I hope you found it useful! If you have any questions, [please send an email to the DataLab](mailto:datalab@ucdavis.edu) and we'll get back to you!
