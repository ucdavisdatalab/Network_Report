...
# make directed flag graph or digraph
.directed_flag = ifelse(.directed, "digraph", "graph")
...
## make network!
rgraph = network(edges, directed = .directed, matrix.type = "edgelist", multiple = .multiplex)
...
rgraph%v%"norm_betweenness" = betweenness(dat = rgraph, gmode = .directed_flag, rescale = TRUE, cmode = ifelse(.directed, "directed", "undirected"))
## eigenvector
rgraph%v%"evc" = evcent(rgraph, gmode = .directed_flag, rescale = TRUE)
...
Explanation Text {data-width=600}
--------------------------------------------------------------------------------
...
| Nodes | `r nrow(nodes)` |
| Edges | `r nrow(edges)` | 
| Density | `r round(gden(rgraph), digits = 3)` | 
| Degree Centralization  | `r round(centralization(rgraph, degree, mode = .directed_flag), digits = 3)` |
| Betweenness Centralization | `r round(centralization(rgraph, betweenness, mode = .directed_flag), digits = 3)` |
| Eigenvector Centralization | `r round(centralization(rgraph, evcent, mode = .directed_flag), digits = 3)` |
| Directed | `r ifelse(.directed, emo::ji("check"), emo::ji("x"))` |
| Weighted Edges| `r ifelse(rgraph$gal$multiple, emo::ji("check"), emo::ji("x"))` |
...
Edges in most network are un-weighted, such that edges either exist or they do not. It is possible to have weighted edges in a network, such that some edges are considered more imprtant than others. This is often used to represent multiple interactions. For example if your network is composed of people, an un-weighted edge might denote two individuals as freinds, whereas a weighted edge may indicate how many time two individuals spend time together; in this situation an edge weight of 1 would be one meeting, while and edge weight of 5 would be five meetings.
...
overview_net$title = paste0("<table>",
                              ...
                              "<tr>",
                                ...
                                "<td>", round(overview_net$mean_distance, digits = 3), "</td>",
                              "</tr>",
                              "<tr>",
...
This network has a total of `r nrow(edges)` edges. The network on the right has the nodes scaled such that larger nodes have a higher total degree. The node highlighted in yellow in the right has the highest total degree at `r max(nodes$degree)` (`r round((max(nodes$degree)/nrow(edges))*100, digits = 3)`% of all edges). The node(s) highlighted in grey has the lowest total degree at `r min(nodes$degree)`
...
The mean betweenness centrality in this network is `r round(mean(nodes$norm_betweenness), digits = 3)`. The network on the right has the nodes scaled such that larger nodes have a larger betweenness centrality. The node highlighted in gold on the right has the largest betweenness centrality at `r max(nodes$norm_betweenness)`; this node likely acts as a gatekeeper in the network. Imagine this node were removed, how would resources need to flow differently in the network without it? The grey node has the smallest betweenness centrality at `r min(nodes$norm_betweenness)`; the network would likely be unaffected by their removal in terms of resource flows.  
...
The mean eigenvector centrality in this network is `r round(mean(nodes$evc), digits = 3)`. The network on the right has the nodes scaled such that larger nodes have a larger eigenvector centrality. The node highlighted in gold on the right has the largest eigenvector centrality at `r max(nodes$evc)`; this node may not be the most central node in the network, but it is connected to many important partners. The grey node has the smallest eigenvector centrality at `r min(nodes$evc)`.

This network has a eigenvector centralization of `r round(centralization(rgraph, evcent, mode = "graph"), digits = 3)`. Centralization is a measure of how unevenly node level metrics are distributed in a network. Scores closer to 0 mean the metric is evenly distributed, while scores closer to 1 indicate a concentration of the metric with a few nodes.
