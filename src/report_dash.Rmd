...
```{r echo=FALSE}
bet_clust = cluster_edge_betweenness(rgraph, weights = if(.multiplex){E(rgraph)$weight} else {NA}, directed = .directed)
```

Betweenness tries to find the nodes that act as gatekeepers or bridges in a network. By gradually removing the edges with the highest betweenness, we can hierarchically see what clusters of nodes are most strongly densely connected with each other.
...
Label Propagation: "In our algorithm every node is initialized with a unique label and at every step each node adopts the label that most of its neighbors currently have. In this iterative process densely connected groups of nodes form a consensus on a unique label to form communities."

#### Community Plot

```{r echo=FALSE}
plot(lab_clust, rgraph, edge.width = if(.multiplex){E(rgraph)$weight} else {NULL})
```
...
melt_nodes = do.call(rbind, by(data = melt_nodes, INDICES = list(as.factor(melt_nodes$group), as.factor(melt_nodes$variable)), FUN = function(mes){
  data.frame("group" = mes$group[1], "variable" = mes$variable[1], "value" = mean(mes$value), stringsAsFactors = FALSE)
  }))
